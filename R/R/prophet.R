...
# TODO decide floor should be by category?
...
initialize_scales_fn <- function(m, initialize_scales, df) {
  ...
  m$y.scale <- by(df, df$category, function(df) {
    sc <- max(abs(df$y - floor))
    if (sc == 0) {
      sc <- 1
    }
  })
  ...
  set_changepoints <- function(m) {
    if (!is.null(m$changepoints)) {
      if (length(m$changepoints) > 0) {
        ...
        if (min(m$changepoints) < min(m$history.dates)
          || max(m$changepoints) > max(m$history.dates)) {
      ...
      if (m$n.changepoints > 0) {
        ...
        m$changepoints <- m$history.dates[cp.indexes]
      } else {
        m$changepoints <- c()
      }
    }
  ...
  fit.prophet <- function(m, df, ...) {
    ...
    stan_init <- function() {
      list(k_category = kinit[1],
         m_category = kinit[2],
         delta_category = matrix(0, dat$C, length(m$changepoints.t)),
  ...
  make_future_dataframe <- function(m, periods, freq = 'day',
    ...
    newdf <- do.call("rbind", by(m$history, m$history$category,
    function(df) {
      dates <- seq(max(df$ds), length.out = periods + 1, by = freq)
      dates <- dates[2:(periods + 1)] # Drop the first, which is max(history$ds)
      category <- rep(df$category[1], length(dates))
      if (include_history) {
        dates <- c(df$ds, dates)
        attr(dates, "tzone") <- "GMT"
        category <- c(df$category, category)
      }
      data.frame(ds = dates, category = category)
    }
  ))
    return(newdf)
  }
